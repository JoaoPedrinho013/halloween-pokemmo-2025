<h1 align="center">Halloween PokeMMO 2025</h1>

<p align="center">
  <img src="https://img.shields.io/badge/React-2025-blue?logo=react&style=for-the-badge">
  <img src="https://img.shields.io/badge/Material%20UI-MUI-007FFF?logo=mui&style=for-the-badge">
  <img src="https://img.shields.io/badge/Halloween%20Theme-Active-orange?style=for-the-badge">
  <img src="https://img.shields.io/badge/Responsive-100%25-success?style=for-the-badge">
</p>

<p align="center">
  Este repositório contém o site criado para apresentar meus resultados do evento Halloween do PokeMMO 2025.<br>
  Inclui estatísticas completas, total vendido, quantidade farmada, prints e informações gerais sobre o farm.
</p>

<hr>

<h2>Funcionalidades do Site</h2>

<h3>1. Página Inicial / Hero Section</h3>
<ul>
  <li>Logo temático com sprite do Pumpking</li>
  <li>Título estilizado "Halloween PokeMMO 2025"</li>
  <li>Subtítulo descritivo</li>
  <li>Botão para download oficial do PokeMMO com efeitos de hover</li>
  <li>Links para GitHub, LinkedIn, WhatsApp e Fórum do Evento</li>
</ul>

<h3>2. Tabela Completa do Farm (Desktop)</h3>
<ul>
  <li>Tabela exibindo todos os itens farmados:</li>
  <ul>
    <li>Pumpkin Goodie Bag</li>
    <li>Mysterious Dusk Ball</li>
    <li>Special Candy</li>
    <li>Spooky Candy</li>
    <li>Goodie Bag</li>
  </ul>
  <li>Exibição de total, média, mínimo, máximo e total vendido</li>
  <li>Linha especial de TOTAL GERAL</li>
  <li>Slide adicional com informações gerais do farm</li>
</ul>

<h3 align="center">Print da tabela (Desktop)</h3>
<img src="public/images/site/site-desktop.png" alt="Print da tabela" width="700">


<hr>

<h2>Responsividade (Mobile)</h2>

<h3>1. Cards Individuais</h3>
<p>Cada item aparece em seu próprio card, mostrando total, média, mínimo, máximo e total vendido.</p>

<h3>2. Card de Total Geral</h3>
<p>Inclui:</p>
<ul>
  <li>Total geral de itens: 1.067</li>
  <li>Total vendido: ¥27.229.600</li>
</ul>

<h3>3. Hero otimizado</h3>
<p>Versão adaptada com sprite menor, título ajustado e botão responsivo.</p>

<h3 align="center">Print do Hero (Mobile)</h3>
<img src="public/images/site/inicio-cards-mobile.png" alt="Print da tabela" width="700">

<h3 align="center">Prints dos Cards (Mobile)</h3>
<img src="public/images/site/cards-mobile.png" alt="Print da tabela" width="700">

<h3 align="center">Print do Total Geral (Mobile)</h3>
<img src="public/images/site/fim-cards-mobile.png" alt="Print da tabela" width="700">

<hr>


<div align="center">
<h2>Dados do Evento</h2>

<table>
  <tr>
    <th>Item</th>
    <th>Total</th>
    <th>Média</th>
    <th>Mín</th>
    <th>Máx</th>
    <th>Total Vendido</th>
  </tr>

  <tr>
    <td>Pumpkin Goodie Bag</td>
    <td>802</td>
    <td>¥22.200</td>
    <td>¥18.000</td>
    <td>¥37.000</td>
    <td>¥17.244.100</td>
  </tr>

  <tr>
    <td>Mysterious Dusk Ball</td>
    <td>119</td>
    <td>¥49.533</td>
    <td>¥46.000</td>
    <td>¥61.000</td>
    <td>¥5.729.400</td>
  </tr>

  <tr>
    <td>Special Candy</td>
    <td>68</td>
    <td>¥50.150</td>
    <td>¥34.000</td>
    <td>¥69.000</td>
    <td>¥3.072.100</td>
  </tr>

  <tr>
    <td>Spooky Candy</td>
    <td>48</td>
    <td>¥17.475</td>
    <td>¥13.400</td>
    <td>¥23.000</td>
    <td>¥836.100</td>
  </tr>

  <tr>
    <td>Goodie Bag</td>
    <td>30</td>
    <td>¥11.060</td>
    <td>¥10.000</td>
    <td>¥13.000</td>
    <td>¥347.900</td>
  </tr>

  <tr>
    <td><b>Total Geral</b></td>
    <td><b>1.067</b></td>
    <td>—</td>
    <td>—</td>
    <td>—</td>
    <td><b>¥27.229.600</b></td>
  </tr>
</table>

</div>


<hr>

<h2>Tecnologias Utilizadas</h2>
<ul>
  <li>HTML5</li>
  <li>CSS3</li>
  <li>JavaScript</li>
  <li>React</li>
  <li>Material UI (MUI)</li>
  <li>MUI Icons</li>
  <li>Layout totalmente responsivo</li>
  <li>Tema visual baseado no evento de Halloween</li>
</ul>

<hr>

<h2>Links Importantes</h2>
<ul>
  <li><a href="https://github.com/JoaoPedrinho013/halloween-pokemmo-2025" target="_blank" rel="noopener noreferrer">GitHub</a></li>
  <li><a href="https://www.linkedin.com/in/joao-pedro-428b43271" target="_blank" rel="noopener noreferrer">LinkedIn</a></li>
  <li><a href="https://wa.me/+5513981309727" target="_blank" rel="noopener noreferrer">WhatsApp</a></li>
  <li><a href="https://forums.pokemmo.com/index.php?/topic/192933-changelog-halloween-2025" target="_blank" rel="noopener noreferrer">Fórum do Evento</a></li>
  <li><a href="https://pokemmo.com/downloads/" target="_blank" rel="noopener noreferrer">Download oficial do PokeMMO</a></li>
</ul>


<hr>

<h2>Identidade Visual</h2>
<ul>
  <li>Tema escuro (dark theme)</li>
  <li>Gradientes roxo para laranja</li>
  <li>Sprites do evento</li>
  <li>Cards com bordas arredondadas</li>
  <li>Fontes com destaque para títulos</li>
</ul>
